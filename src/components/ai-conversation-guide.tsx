...
import { LucideBrain, LucideMic, LucideMessageSquare, LucideArrowRight } from "lucide-react"

// Sample conversation flow for health assessment
const conversationFlow = [
  {
    id: "greeting",
    message: "Hello, I'm here to help you describe your health concerns. What's your main health issue today?",
  ...
  {
    id: "triggers",
    message: "Have you noticed anything that triggers or worsens your symptoms?",
    expectsResponse: true,
  },
  {
  ...
  {
    id: "medical-history",
    message: "Do you have any relevant medical history or conditions I should know about?",
    expectsResponse: true,
  },
  {
    id: "summary",
    message: "Thank you for sharing this information. I'll summarize what you've told me so far.",
...
export function AIConversationGuide({ onComplete, className = "" }: AIConversationGuideProps) {
  ...
  return (
    <Card className={`${className}`}>
      <CardHeader>
        ...
        <CardDescription>Let our AI guide you through describing your health concerns</CardDescription>
      </CardHeader>
      <CardContent className="h-[400px] overflow-y-auto">
        <div className="space-y-4">
          {conversationFlow.slice(0, currentStep + 1).map((step, index) => (
            <div key={step.id} className={index === currentStep && isTyping ? "opacity-70" : ""}>
              {/* AI message */}
              ...
              {step.expectsResponse &&
                (index < currentStep || (!isTyping && index === currentStep && responses[step.id])) && (
                  <div className="flex gap-3 mb-4 justify-end">
                    <div className="rounded-lg bg-primary text-primary-foreground p-3 max-w-[80%]">
                      <p className="text-sm">{responses[step.id] || currentResponse}</p>
                    </div>
      ...
      <CardFooter className="flex flex-col gap-4 border-t pt-4">
        {currentStep < conversationFlow.length - 1 && conversationFlow[currentStep].expectsResponse && !isTyping && (
        ...
        <div className="flex justify-between w-full">
          <Button
            ...
            className={`rounded-full ${isListening ? "bg-destructive text-destructive-foreground" : ""}`}
            onClick={(e) => {
          ...
          <Button
            onClick={handleNext}
            disabled={isTyping || (conversationFlow[currentStep].expectsResponse && !currentResponse.trim())}
