...
export default function LoginPage() {
  ...
  const handleLogin = async (userType: "patient" | "clinician") => {
    ...
    try {
      ...
      await signIn(email || "demo@symedon.com", password || "password", userType)
    } catch (err) {
      console.error(err)
    ...
    <div className="container max-w-md py-12 px-4">
      ...
      <div className="flex flex-col items-center mb-8">
        <div className="relative h-16 w-16 overflow-hidden mb-4">
          <Image src="/images/symedon-logo.png" alt="Symedon Logo" width={64} height={64} className="object-contain" />
        </div>
        ...
        <TabsContent value="clinician">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Enter your credentials to access your clinical dashboard</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
            ...
            <CardFooter className="flex flex-col gap-3">
              <Button className="w-full" onClick={() => handleLogin("clinician")} disabled={isLoading}>
                {isLoading ? "Signing in..." : "Sign In"}
              </Button>
              <p className="text-sm text-center text-muted-foreground">
                Don't have an account?{" "}
                <Link href="/clinician/register" className="text-primary underline">
